...
///
/// A `Protocol` is spawned using the [`spawn`](crate::spawn) function. When spawned, the process
...
/// will fail to compile. Protocoles don't share any memory and everything needs to be passed
/// through a message. This also limits the capturing process to only types that can be
/// de/serialized with the serializer `S`.
///
/// A message can be sent to the `Protocol` with the [`send`](Protocol::send) method.
///
/// # Example
///
/// ```
/// let proc = spawn::<Protocol<_>, _>(capture, |_capture, mailbox: Mailbox<i32>| {
///   let received_value = mailbox.receive();
/// });
/// proc.send(1);
/// ```
pub struct Protocol<M, S = Bincode>
where
    S: Serializer<M>,
{
    id: u64,
...
impl<M, S> Resource for Protocol<M, S>
where
    S: Serializer<M>,
{
    fn id(&self) -> u64 {
...
impl<C, M, S> IntoProcess<C> for Protocol<M, S>
where
...
impl<C, M, S> IntoProcessLink<C> for Protocol<M, S>
where
    S: Serializer<C> + Serializer<M>,
{
    type Handler = fn(capture: C, arg: Mailbox<M, S>);
    ...
    ) -> Result<Protocol<M, S>, LunaticError>
    where
        Self: Sized,
    {
    ...
    S: Serializer<C> + Serializer<M>,
{
    ...
    if result == 0 {
        ...
        if std::mem::size_of::<C>() == 0 {
            ...
            let child = Protocol::<C, S> {
                id,
                consumed: UnsafeCell::new(false),
                serializer_type: PhantomData,
            };
            child.send(captured);
            // Protocoles can only receive one type of message, but to pass in the captured variable
...
// Protocoles are equal if their UUID is equal.
impl<M, S> PartialEq for Protocol<M, S>
where
    S: Serializer<M>,
{
...
impl<M, S> Clone for Protocol<M, S>
where
    S: Serializer<M>,
{
    fn clone(&self) -> Self {
        let id = unsafe { host_api::process::clone_process(self.id) };
        unsafe { Protocol::from_id(id) }
    }
}

impl<M, S> Drop for Protocol<M, S>
where
    S: Serializer<M>,
{
    fn drop(&mut self) {
...
impl<M, S> serde::Serialize for Protocol<M, S>
where
    S: Serializer<M>,
{
...
struct ProtocolVisitor<M, S> {
    _phantom: PhantomData<(M, S)>,
}

impl<'de, M, S> serde::de::Visitor<'de> for ProtocolVisitor<M, S>
where
    S: Serializer<M>,
{
    type Value = Protocol<M, S>;
        ...
        E: serde::de::Error,
    {
        ...
        Ok(unsafe { Protocol::from_id(id) })
    }
}

impl<'de, M, S> serde::de::Deserialize<'de> for Protocol<M, S>
where
    S: Serializer<M>,
{
    fn deserialize<D>(deserializer: D) -> Result<Protocol<M, S>, D::Error>
    where
        D: serde::de::Deserializer<'de>,
    {
        deserializer.deserialize_u64(ProtocolVisitor {
...
mod tests {
    ...
    #[test]
    fn spawn_test() {
        let child = spawn::<Protocol<i32>, _>(1, |capture, mailbox| {
            assert_eq!(capture, 1);
    ...
    fn spawn_link_test() {
        ...
        spawn::<Protocol<i32>, _>((), |_, _| {
            spawn_link::<Protocol<i32>, _>((), |_, _| {
                panic!("fails");
            })
            .unwrap();
